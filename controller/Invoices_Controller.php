...
class Invoices_Controller
{
    ...
    public function Consult($array)
    {
        ...
        $fechaDia = date('Y-m-d H:i:s', time());
        $fechaDia1 = date('Y-m-d 00:00:00', time());
        $fechaDia2 = date('Y-m-d 23:00:00', time());

        if ($array[0] == 1) {
            $conexion = Conexion::connection();
           
            $sql = "Select fa.IdFactura,fa.Creacion,cl.Nombre1,fa.Total,fa.Monto_P,fa.Monto_D,fa.Medidas from facturas fa INNER JOIN clientes cl INNER JOIN usuarios us where fa.Estado != 0 and fa.IdCliente = cl.IdCliente and fa.IdUsuario= us.IdUsuario ORDER BY fa.IdFactura DESC LIMIT 0,200"; // AND fa.fecha BETWEEN '$fechaDia1' AND '$fechaDia2'
            return $conexion->query($sql);
        }
        if ($array[0] == 2) {
            $conexion = Conexion::connection();
            $sql = "Select fa.IdFactura,fa.Creacion,cl.Nombre1,fa.Total,fa.Monto_P,fa.Monto_D,fa.Medidas from facturas fa INNER JOIN clientes cl INNER JOIN usuarios us where fa.IdCliente = cl.IdCliente and fa.IdUsuario= us.IdUsuario AND fa.IdUsuario='$array[1]' ORDER BY fa.IdFactura DESC LIMIT 0,200"; //AND fa.fecha BETWEEN '$fechaDia1' AND '$fechaDia2'
            return $conexion->query($sql);
        }
        $conexion = Conexion::connection();
        $id = $array[1];

        $result1 = $conexion->query("Select fa.IdFactura,fa.Creacion,fa.Total,fa.Monto_P,fa.Monto_D,cl.Nombre1,cl.Celular,fa.Medidas  from facturas fa INNER JOIN clientes cl where fa.IdCliente=cl.IdCliente AND fa.IdFactura='$id'");
    ...
    {   
        ...
        $cantTiras=$array['cantidadTiras'][0];
         $producName=$array['productosTiras'][0];

        $conexion = Conexion::connection();

            
        // for ($i = 0; $i < count($array['cantidadTiras']); $i++) {
        ...
        for ($i = 0; $i < count($array['cantidadTiras']); $i++) {
            $cantTiras=$array['cantidadTiras'][$i];
            $producName=$array['productosTiras'][$i];

            $sql = "SELECT Nombre FROM productos WHERE idProducto='$producName'";
            $resultado3 = $conexion->query($sql);
            $resultado3 = $resultado3->fetch_row();


            date_default_timezone_set('America/Bogota');
            $date1 = date('Y-m-d', time());
            $stmt8=("INSERT tiras (CantidadTiras,RefTiras,Fecha) VALUES('$cantTiras','$resultado3[0]','$date1')");
            $resultado3 = $conexion->query($stmt8);

            $stmt9 =("UPDATE productos SET Cantidad = (SELECT Cantidad-'$cantTiras') WHERE idProducto='$producName'");
            $resultado3 = $conexion->query($stmt9);
        }
        ...
        $totalTiras = 0;
        foreach ($array["cantidadTiras"] as $key => $value) {
            $totalTiras = $totalTiras + $value;
        }
        echo array_search("tiras",array_keys($array));

        date_default_timezone_set('America/Bogota');
        $date = date('Y-m-d H:i:s', time());
        $stmt = $conexion->prepare("INSERT INTO facturas(IdCliente,Fecha,IdUsuario,Total,Creacion,Monto_P,Monto_D,Tiras,Medidas) VALUES(?,'$date',?,?,'$date',?,?,?,?)");
        $stmt->bind_param("iidddis", $array['companyName'], $array['userId'], $array['subTotal'],$array['amountPaid'],$array['amountDue'],$totalTiras,$array['notes']);
        $stmt->execute();
        ...
        header("Refresh: 0.5; URL='Create_Invoice.php'");
        return $stmt;
    }
    ...
    public function Delete($array, $count = 0)
    {
        ...
        if ($count == 0) {
            ...
            $dateTime = $conexion->query("SELECT IF('$fecha' < ADDDATE(Creacion, INTERVAL 24 hour)
