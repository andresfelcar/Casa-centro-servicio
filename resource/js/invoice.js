...
function selectTiras(id, idSelect) {
    var result;
    $.ajax({
        type: "POST",
        data: "idProduct=" + id,
...
$(document).ready(function() {
    ...
    $(document).on('click', '#addRows', function() {
        ...
        htmlRows += '<td><select name="productosTiras[]" id="productosTiras_' + count + '" class="form-control"onchange="javascript:selectTiras(this.value,' + count + ');"><option selected>Seleccione alguno</option>' + datos + '</select></td>';
        htmlRows += '<td><input type="number" name="cantidadTiras[]" id="cantidadTiras_' + count + '" class="form-control total" onchange="javascript:validateQuantity(this.value,' + count + ');"></td>';
        $('#invoiceItem').append(htmlRows);
        $('#productCode_' + count).select2();
        $('#productosTiras_' + count).select2();
    });
